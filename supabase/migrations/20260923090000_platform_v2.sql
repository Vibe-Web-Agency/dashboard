-- ═════════════════════════════════════════════════════════════════════════
--  Migration initiale — plateforme v2
--
--  Schéma complet : 63 tables, 2 vues, les fonctions et les 122 politiques
--  RLS. Conçu domaine par domaine dans supabase/design/schema-v2.sql, qui
--  reste le document de référence commenté.
--
--  Vérifié par `npm run db:check` (257 scénarios dans un Postgres 17).
--  Ne modifie RIEN de l'ancienne base : c'est un schéma neuf, destiné au
--  projet de dev puis à la future production.
-- ═════════════════════════════════════════════════════════════════════════
...
create schema if not exists extensions;   -- déjà présent chez Supabase ; utile en local
create extension if not exists btree_gist with schema extensions;
-- La contrainte d'exclusion de `shifts` référence une classe d'opérateurs de
-- btree_gist : sans ce search_path, Supabase ne la trouve pas.
